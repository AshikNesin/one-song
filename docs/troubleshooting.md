...
| `SecurityException` on playback | File URI lost permission after reinstall | App stores the original picked URI. If it becomes invalid, the app detects the failure on launch and navigates to onboarding so the user can reselect. |
